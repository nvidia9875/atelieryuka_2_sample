...
(function () {
  ...
  /* ---------- 6. 衣裳店デスクのツール ---------- */
  /* 統合ログインは提案から外した。各ツールはワンクリックで直接開く */
  function openDeskTool(key) {
    if (key === "catalog") { if (window.WWCatalog) window.WWCatalog.open(); return; }
    if (key === "avail") { openAvail(); return; }
    if (key === "reserve") { openReserve(); return; }
    if (key === "assets") { $("#assets-dialog").showModal(); }
  }

  $$("[data-desk]").forEach(function (btn) {
    btn.addEventListener("click", function () {
      openDeskTool(btn.getAttribute("data-desk"));
    });
  });
  ...
  /* 品番と日付から決定的に印を作る。毎回変わると説明中に話が食い違うため乱数は使わない */
  function availMark(code, d) {
    var s = 0;
    for (var i = 0; i < code.length; i++) s += code.charCodeAt(i);
    var v = (s * 31 + d.getFullYear() * 372 + (d.getMonth() + 1) * 37 + d.getDate() * 7) % 11;
  ...
  function buildCalendar(code) {
    ...
    nameEl.textContent = p
      ? "品番 " + code + " — " + productName(p)
      : "品番を入力すると、その一着の空き状況を表示します。";
    if (!p) return;

    var today = new Date();
    today.setHours(12, 0, 0, 0);
    var start = addDays(today, -((today.getDay() + 6) % 7)); // 今週の月曜
    for (var w = 0; w < 4; w++) {
      var tr = document.createElement("tr");
      for (var i = 0; i < 7; i++) {
        var d = addDays(start, w * 7 + i);
        var td = document.createElement("td");
        var past = d.getTime() < today.getTime();
        var label = (d.getDate() === 1 || (w === 0 && i === 0))
          ? (d.getMonth() + 1) + "/" + d.getDate() : String(d.getDate());
        var dayEl = document.createElement("span");
        dayEl.className = "cal-d";
        dayEl.textContent = label;
        var mark = document.createElement("span");
        var sr = document.createElement("span");
        sr.className = "cal-sr";
        if (past) {
          td.className = "is-past";
          mark.className = "cal-s";
          mark.textContent = "−";
          sr.textContent = label + " 受付終了";
        } else {
          var m = availMark(code, d);
          mark.className = "cal-s " + m[1];
          mark.textContent = m[0];
          sr.textContent = label + " " + m[2];
        }
  ...
  function openAvail() {
    if (!avDialog) return;
    buildCalendar((avCode.value || "").replace(/[^0-9]/g, ""));
    avDialog.showModal();
  }
  if (avCode) {
    avCode.addEventListener("input", function () {
      buildCalendar(avCode.value.replace(/[^0-9]/g, ""));
    });
  }

  /* ---------- 14. 衣裳店デスク Ⅱ: WEB予約（デモ・3ステップ） ---------- */
  var rvDialog = $("#reserve-dialog");
  var rvSched = null;

  function wizGo(step) {
    $$(".wiz-pane", rvDialog).forEach(function (pane) {
      pane.hidden = pane.getAttribute("data-wp") !== String(step);
    });
    $$("#wiz-steps li").forEach(function (li, i) {
  ...
  function openReserve() {
    if (!rvDialog) return;
    $("#rv-done").hidden = true;
    $$(".wiz-pane", rvDialog).forEach(function (p) { p.hidden = false; });
    wizGo(1);
    rvDialog.showModal();
  }

  if (rvDialog) {
    $$("[data-wnext]", rvDialog).forEach(function (btn) {
      btn.addEventListener("click", function () {
        var next = parseInt(btn.getAttribute("data-wnext"), 10);
        if (next === 2) {
          var code = $("#rv-code").value.replace(/[^0-9]/g, "");
          var date = $("#rv-date").value;
          var err = $("#rv-err1");
          if (!findProduct(code)) { err.hidden = false; err.textContent = "実在する品番をご入力ください（例: 20432）。"; return; }
          var r = WW.schedule(date);
          if (!r.ok) { err.hidden = false; err.textContent = r.reason; return; }
          err.hidden = true;
          rvSched = r;
          var dl = $("#rv-sched");
          dl.hidden = false;
          dl.replaceChildren();
          [["WEB予約締切", r.deadline], ["出荷日", r.ship], ["お届け日", r.arrive], ["ご返送日", r.ret]].forEach(function (row) {
            var div = document.createElement("div");
        ...
        if (next === 3) {
          var shop = $("#rv-shop").value.trim();
          var staff = $("#rv-staff").value.trim();
          var tel = $("#rv-tel").value.trim();
          var err2 = $("#rv-err2");
          if (!shop || !staff || tel.replace(/[^0-9]/g, "").length < 10) {
            err2.hidden = false;
            err2.textContent = "店舗名・ご担当者・お電話番号（10桁以上）をご入力ください。";
            return;
          }
          err2.hidden = true;
          var conf = $("#rv-confirm");
          conf.replaceChildren();
          var p = findProduct($("#rv-code").value.replace(/[^0-9]/g, ""));
          addDlRow(conf, "品番", p ? p.code + " " + productName(p) : "");
          addDlRow(conf, "ご使用日", rvSched ? rvSched.use : "");
          addDlRow(conf, "予約締切", rvSched ? rvSched.deadline : "");
          addDlRow(conf, "店舗名", shop);
          addDlRow(conf, "ご担当者", staff);
          addDlRow(conf, "お電話", tel);
        }
        wizGo(next);
      });
    });
    $$("[data-wback]", rvDialog).forEach(function (btn) {
      btn.addEventListener("click", function () { wizGo(parseInt(btn.getAttribute("data-wback"), 10)); });
    });
    $("#rv-send").addEventListener("click", function () {
      $$(".wiz-pane", rvDialog).forEach(function (p) { p.hidden = true; });
      var done = $("#rv-done");
      done.hidden = false;
      $("#rv-done-t").textContent = rvSched
        ? "ご使用日 " + rvSched.use + " ／ 出荷 " + rvSched.ship + " ／ お届け " + rvSched.arrive
        : "";
      done.focus();
    });
  }
  ...
  /* ---------- 15. 衣裳店デスク Ⅳ: 販促素材（デモ） ---------- */
  $$("[data-asset]").forEach(function (btn) {
    btn.addEventListener("click", function () {
      $("#asset-done").textContent = "「" + btn.getAttribute("data-asset") + "」のダウンロードを開始しました（デモ）。";
    });
  });
